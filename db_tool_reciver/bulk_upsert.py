...
def coerce_lookup_fields(lookup_fields: Any) -> list[str]:
...
def allowed_fields(doctype: str) -> set[str]:
...
def existing_map(doctype: str, key_field: str, keys: list[str], chunk_size: int = DEFAULT_EXISTING_CHUNK) -> dict[str, str]:
	out: dict[str, str] = {}
	if not keys:
		return out
...
def lookup_signature(payload: dict[str, Any], lookup_fields: list[str]) -> str:
	parts = [cstr(payload.get(field) or "").strip() for field in lookup_fields]
	return "||".join(parts)


def find_existing_by_lookup(doctype: str, payload: dict[str, Any], lookup_fields: list[str]) -> str | None:
	filters = []
...
def bulk_upsert_impl(
	...
	allowed = allowed_fields(doctype)
	key_field = cstr(key_field or "").strip() or None
	lookup_fields = [field for field in (lookup_fields or []) if field]
	...
	keys: list[str] = []
	for row in rows:
		payload = normalize_payload(row, allowed)
	...
	for idx, payload in enumerate(cleaned_rows, start=1):
		key_value = cstr(payload.get(key_field) or "").strip() if key_field else ""
		lookup_sig = lookup_signature(payload, lookup_fields) if lookup_fields else ""
		try:
			...
			if not existing_name and lookup_fields:
				...
				if not existing_name:
					existing_name = find_existing_by_lookup(doctype, payload, lookup_fields)
					if existing_name:
			...
			if existing_name:
				...
				for fieldname, value in payload.items():
					if fieldname in {"name", key_field}:
						continue
